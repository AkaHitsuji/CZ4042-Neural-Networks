...
# y_mean = np.mean(cal_housing, axis=0)[-1]
# y_std = np.std(cal_housing, axis=0)[-1]
...
regularization = tf.nn.l2_loss(V) + tf.nn.l2_loss(W) + tf.nn.l2_loss(V2)
...
fig = plt.figure(1)
plt.title("4 layers With Dropout")
